...
class Snake(object):
    ...
    def addSquare(self):
        ...
        """
        mixer.music.load("Snake/data/eating.wav")  # Eating sound
...
class Button:
    ...
    def draw(self, window):
        ...
        font = pygame.font.Font("Snake/data/8-bit-pusab.ttf", 20)
...
def redraw(window, snake, snack, score):
    ...
    font = pygame.font.Font("Snake/data/8-bit-pusab.ttf", 20)  # Font used to display score
...
def helpWindow(window):
    ...
    """
    run = True
    font = pygame.font.Font("Snake/data/8-bit-pusab.ttf", 18)  # Font used to display score
...
def highScoreWin(window):
    ...
    titleFont = pygame.font.Font("Snake/data/8-bit-pusab.ttf", 36)
    font = pygame.font.Font("Snake/data/8-bit-pusab.ttf", 24)
    resetFont = pygame.font.Font("Snake/data/8-bit-pusab.ttf", 14)
    HSTitle = titleFont.render("High Score:", True, (255, 255, 255))
    resetText = resetFont.render("Reset", True, (255, 255, 255))
    resetIcon = pygame.image.load("Snake/data/resetIcon.png")
    ...
    while run:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if back.hover(mousePos):
                    ...
                    file = open("Snake/data/highscores.txt")
                    lines = file.readlines()
                    lines[0] = "0\n"
                    file = open("Snake/data/highscores.txt", "w")
        ...
        HSFile = open("Snake/data/highscores.txt")
...
def mainMenu():
    ...
    pygame.display.set_caption("Snake — Feito por Mateus, Letícia, João e Diego")  # Sets caption of window
    icon = pygame.image.load("Snake/data/Snakeicon.png")
    ...
    # Reads High Scores
    HSFile = open("Snake/data/highscores.txt", "r+")
    ...
    while run:
        ...
        window.fill((47, 48, 47))  # Fills background with a GRAY color
        font = pygame.font.Font("Snake/data/8-bit-pusab.ttf", 60)  # Font used to display score
        font2 = pygame.font.Font("Snake/data/8-bit-pusab.ttf", 16)  # Font used to display score
        ...
        qMark = pygame.image.load("Snake/data/qMark.png")
        qMark = pygame.transform.scale(qMark, (32, 32))
        trophy = pygame.image.load("Snake/data/trophy.png")
...
def main(window, diff, high):
    ...
    while run:
        ...
        if secondSquare or snakeHit(snake):  # Checks if a Snake has collided with itself
            mixer.music.load("Snake/data/thud.wav")  # Hit itself sound effect
            mixer.music.play()
            run = False
...
def gameOver(window, score, diff, high):
    ...
    while run:
        ...
        if score > high:
            ...
            HSFile = open("Snake/data/highscores.txt", "w")
            HSFile.writelines(lines)
            HSFile.close()

        window.fill((47, 48, 47))  # Fills background with a GRAY color
        mousePos = pygame.mouse.get_pos()
        font = pygame.font.Font("Snake/data/8-bit-pusab.ttf", 40)  # Font used to display score
        font2 = pygame.font.Font("Snake/data/8-bit-pusab.ttf", 21)  # Font used to display score
        font3 = pygame.font.Font("Snake/data/8-bit-pusab.ttf", 18)  # Font used to display score
        ...
        back.draw(window)
        arrow = pygame.image.load("Snake/data/back.png")
